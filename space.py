...
custom_space = int(input("Enter custom space: "))
...
for space in words:
    word_segment += space + ' '
    if len(word_segment.split()) == custom_space:
        current_segment.append([segment.strip() for segment in word_segment.split(space_break)])
